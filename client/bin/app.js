...
function secondsToStr(seconds, allowNegative) {
    if (allowNegative === void 0) { allowNegative = false; }
    var isNegative = seconds < 0;
    var clampedSeconds = Math.abs(allowNegative ? seconds : Math.max(0, seconds));
    ...
    var mlz = m < 10 ? "0" + String(m) : String(m);
...
function setTimeToElem(elem, seconds, allowNegative) {
    if (allowNegative === void 0) { allowNegative = false; }
...
var TimeToCurl = (function () {
            ...
            return tslib_1.__generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        this.allPresets = presets_1.TimerPresets.concat(window.ttcPlugins
                            .filter(function (p) { return !!p.presets; })
                            .map(function (p) { return p.presets; })
                            .reduce(function (a, b) {
                            return a.concat(b);
                        }, []));
                        return [4 /*yield*/, this.loadTimers(util_1.getDisplayedTimers())];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
    ...
    TimeToCurl.prototype.populateTimerOptions = function () {
        ...
        for (var _i = 0, _a = this.allPresets; _i < _a.length; _i++) {
            var preset = _a[_i];
            ...
            return tslib_1.__generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        preGameTime = this.simpleInput("Pre Game Time", "preGameTime", simpleOptions.preGameTime);
                        ...
                        allOptionsContainer.addEventListener("input", function () {
                            ...
                            var valPreGameTime = util_1.strToSeconds(preGameTime.children[1].value);
    ...
    TimeToCurl.prototype.setNextTimerOptionsFromDropdown = function () {
        ...
        var matchedPreset = this.allPresets.filter(function (p) { return p.id === dropdownValue; })[0];
        if (matchedPreset) {
    ...
    TimeToCurl.prototype.setUpEvents = function () {
        ...
        document.addEventListener("DOMContentLoaded", function () { return tslib_1.__awaiter(_this, void 0, void 0, function () {
            ...
            return tslib_1.__generator(this, function (_b) {
                ...
                for (_i = 0, _a = window.ttcPlugins; _i < _a.length; _i++) {
                    plugin = _a[_i];
...
var TimeMinder = (function (_super) {
...
var SimpleTimerUI = (function (_super) {
    ...
    SimpleTimerUI.prototype.setNewState = function (state) {
        ...
        var mainTimer = new time_minder_1.TimeMinder(this.state.timeRemaining * this.lengthOfSecond, function () {
            ...
            _this.timerContainerElement.classList.remove("warning", "pregame");
            _this.timerContainerElement.classList.add("no-more-ends");
            _this.pacingElement.classList.add("invisible");
        });
        mainTimer.every(this.lengthOfSecond / 10, function () {
            var timeRemaining = mainTimer.getTimeRemaining() / _this.lengthOfSecond;
            var isPregame = timeRemaining > _this.options.totalTime;
            var renderPacing = _this.options.showPacing && !isPregame;
            var timeToSet = isPregame ? -1 * (timeRemaining - _this.options.totalTime) : timeRemaining;
            var refit = timeToSet !== _this.lastTimeSet;
            _this.lastTimeSet = timeToSet;
            util_1.setTimeToElem(_this.remainingTime, timeToSet, isPregame);
            if (refit) {
                util_1.refitScaledElements();
            }
            // We're in the pre-game phase
            if (isPregame) {
                _this.timerContainerElement.classList.remove("warning", "no-more-ends");
                _this.timerContainerElement.classList.add("pregame");
            }
            else if (timeRemaining <= _this.options.noMoreEndsTime) {
                ...
                if (_this.currentMode !== "warning") {
                    _this.timerContainerElement.classList.remove("no-more-ends", "pregame");
                ...
                _this.currentMode = "normal";
                _this.timerContainerElement.classList.remove("warning", "no-more-ends", "pregame");
            }
            if (renderPacing) {
                _this.pacingElement.classList.remove("invisible");
...
var confirm_1 = __webpack_require__(5);
var time_minder_1 = __webpack_require__(6);
var StandardTimerUI = (function (_super) {
    ...
    function StandardTimerUI(initParams, container, application) {
        ...
        _this.useTimeouts = initParams.options.numTimeouts > 0;
        if (initParams.options.lengthOfSecond) {
    ...
    StandardTimerUI.prototype.initKeyboardShortcuts = function () {
        var _this = this;
        window.addEventListener("keydown", function (event) {
            switch (event.code) {
                case "KeyZ":
                    // Toggle yellow timer
                    _this.sendPhaseTransition("toggle-thinking", { team: "Yellow" });
                    event.preventDefault();
                    break;
                case "KeyQ":
                    // Toggle red timer
                    _this.sendPhaseTransition("toggle-thinking", { team: "Red" });
                    event.preventDefault();
                    break;
                case "KeyC":
                    // Cancel whatever is happening
                    _this.sendPhaseTransition("cancel-timer");
                    event.preventDefault();
                    break;
                case "F1":
                    // Between end break
                    _this.sendPhaseTransition("end-end");
                    event.preventDefault();
                    break;
                case "F5":
                    // Mid-game break
                    _this.sendPhaseTransition("begin-midgame-break");
                    event.preventDefault();
                    break;
            }
        });
    ...
    StandardTimerUI.prototype.setNewState = function (state) {
        ...
        var _loop_3 = function (teamId) {
            ...
            if (this_3.state.phase === "thinking") {
                ...
                if (this_3.state.timeoutsRemaining[thinkingTeam] > 0) {
                    this_3.beginTimeoutButton.disabled = false;
                }
                else {
                    this_3.beginTimeoutButton.disabled = true;
                }
                if (thinkingTeam === teamId) {
                    ...
                    stoneTimer_1.every(this_3.lengthOfSecond / 10, function () {
                        util_1.setTimeToElem(_this.elapsedThinkingTime[teamId], (stoneTimer_1.elapsedTime() + (_this.state.currentTimerRunningTime || 0)) / _this.lengthOfSecond);
                    }, false);
                    stoneTimer_1.start();
    ...
    StandardTimerUI.prototype.initElements = function (template) {
        ...
        if (this.elements["begin-timeout"] && this.elements["begin-timeout"][0]) {
            this.beginTimeoutButton = this.elements["begin-timeout"][0];
        }
        if (this.elements["game-start-warmup"] && this.elements["game-start-warmup"][0]) {
            this.beginWarmupButton = this.elements["game-start-warmup"][0];
        }
    };
...
exports.TimerPresets = [
    ...
    },
    {
        ...
        options: tslib_1.__assign({}, exports.SimpleBaseOptions, { numEnds: 8, totalTime: 90 * 60 }),
        type: "simple" /* Simple */,
    },
    {
        id: "2-hour",
